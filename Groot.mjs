...
class Groot{
    ...
    async init(){
        ...
        try {
            await fs.writeFile(this.headPath, '', {flag:'wx'});//wx: create file if not exist and fails if exist
    ...
    async add(fileToBeAdded){
        const content = await fs.readFile(fileToBeAdded, {encoding:'utf-8'});
        const hash = this.hashObject(content);
        console.log(hash);
        const fileHashedObjectPath = path.join(this.objectsPath, hash);
        await fs.writeFile(fileHashedObjectPath, content);
    ...
    async commit(message){
        ...
        console.log(`Committed as commit ${commitHash}`);
    ...
    async getParentFileContent(parentCommitData, filePath){
        ...
        if(parentFile){
            // get the file from the parent commit and return the content 
